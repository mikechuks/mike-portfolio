...
const Portfolio = () => {
  ...
  // Handle click on mobile to show/hide overlay
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (window.innerWidth <= 768 && activeItemId !== null) {
    ...
    >
          ...
          {filteredItems.map((item, index) => (
            ...
            >
              <div className="relative overflow-hidden">
                ...
                {/* Gradient overlay - shows on hover (desktop) or when clicked (mobile) */}
                ...
                {/* Overlay Content - Slides up on hover (desktop) or when clicked (mobile) */}
                <div className={`absolute inset-0 flex flex-col justify-end p-6 transition-transform duration-500 ${
                  window.innerWidth <= 768
                    ? (activeItemId === item.id ? 'transform translate-y-0' : 'transform translate-y-full')
                    : 'transform translate-y-full group-hover:translate-y-0'
                }`}>
                  <h3 className="text-white text-xl font-bold mb-2">{item.title}</h3>
                  <p className="text-gray-200 text-sm mb-3">{item.description}</p>
                  <div className="flex gap-2 flex-wrap">
                    {/* VIEW PROJECT BUTTON - SOLID GRADIENT, NOT TRANSPARENT */}
                    <a 
                      className="px-4 py-2 bg-white/50 from-purple-600 to-blue-600 rounded-full text-white text-sm font-semibold hover:shadow-lg transition-all transform hover:scale-105 inline-flex items-center gap-1 " 
                      href={item.href} 
                      target="_blank" 
                  ...
                  {window.innerWidth <= 768 && activeItemId === item.id && (
                    <button 
                      className="absolute top-4 right-4 text-white/70 hover:text-white text-sm bg-black/50 rounded-full w-8 h-8 flex items-center justify-center"
                      onClick={(e) => {
                        e.stopPropagation();
                        setActiveItemId(null);
                      }}
                ...
                {/* Category Badge */}
                <div className={`absolute top-4 right-4 px-3 py-1 bg-gradient-to-r ${item.badgeColor} rounded-full text-white text-xs font-semibold transition-transform duration-500 ${
                  window.innerWidth <= 768 && activeItemId === item.id
      ...
      <style jsx>{`
        @media (max-width: 768px) {
          ...
          /* Ensure the active overlay is above everything */
